...
namespace ft {
    ...
    public:
        ...
        explicit map(const key_compare &comp = key_compare(), const allocator_type &allocator = allocator_type())
                : container(node_container(value_compare(comp))), compare(comp), allocator(allocator) {}

        template<class InputIterator>
        map(InputIterator f, InputIterator l, const key_compare &comp = key_compare(),
            const allocator_type &allocator = allocator_type()) : container(node_container(value_compare(comp))),
                                                                  compare(comp), allocator(allocator) {
            insert(f, l);
        }
