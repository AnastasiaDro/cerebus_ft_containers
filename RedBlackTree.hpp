...
namespace ft {
    ...
    public:
        RedBlackTree(const compare_key cmp) : compare(cmp), allocator(allocator_type()), root(NULL), node_p(NULL),
                                              tree_size(0) {}

        RedBlackTree(const RedBlackTree &other) : compare(other.compare), allocator(other.allocator), root(NULL),
                                                  node_p(NULL), tree_size(other.tree_size) {
